...
const PlayerRoomCard = ({ players }: { players: Player[] }) => {
  return (
    <div className="col-span-3 row-span-2 row-start-2 bg-amber-300 rounded-md p-4 shadow-md">
          ...
          {players.length === 0 ? (
            ...
            <ul className="flex space-x-3 h-full">
              {players.map((player, index) => (
                <li
                  key={index}
                  className={`flex flex-col items-center px-4 py-3 rounded-md border min-w-[300px] ${
                    player.isReady
                      ? "border-green-400 bg-green-50"
                      : "border-gray-300 bg-gray-50"
                  }`}
                >
                  {/* Avatar atau icon player (opsional) */}
                  <div className="w-15 h-15 rounded-full bg-amber-400 flex items-center justify-center text-white font-bold mb-2">
